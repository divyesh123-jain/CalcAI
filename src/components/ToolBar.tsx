        ...
        <Popover open={activePopover === label} onOpenChange={(isOpen) => setActivePopover(isOpen ? label : null)}>
          <PopoverTrigger asChild>
            <Button
              ...
              size="icon"
              disabled={disabled}
              className={`h-12 w-12 sm:h-10 sm:w-10 rounded-lg ${isActive ? 'bg-blue-500/20 text-blue-300' : 'text-white/80 hover:bg-white/10'}`}
              onClick={() => {
                onClick?.();
  ...
  return (
    <>
      ...
      <div className="fixed top-4 left-1/2 -translate-x-1/2 z-50 w-[95vw] sm:w-auto">
        <div className="flex items-center gap-4 sm:gap-2 p-3 sm:p-2 rounded-xl backdrop-blur-lg bg-black/40 border border-white/20 shadow-2xl overflow-x-auto no-scrollbar">
          ...
          <div className="w-px h-6 bg-white/20 mx-3 sm:mx-2" />
          
          <Popover>
             <PopoverTrigger asChild>
                <Button
                    style={{ backgroundColor: selectedColor }}
                    className="w-10 h-10 sm:w-8 sm:h-8 rounded-full border-2 border-white/50 transition-transform hover:scale-110"
          ...
          <Button onClick={onCalculate} disabled={isLoading} className="bg-blue-600 hover:bg-blue-700 text-white font-semibold px-4 h-12 sm:h-10 rounded-lg">
      ...
      {/* View Controls Toolbar */}
       <div className="fixed bottom-4 left-1/2 -translate-x-1/2 z-50">
         <div className="flex items-center gap-4 sm:gap-2 p-3 sm:p-2 rounded-xl backdrop-blur-lg bg-black/40 border border-white/20 shadow-2xl">
